...
import 'package:hana_sdk/core/elements/dynamic_slider/dynamic_slider.dart';
...
class DynamicSliderController extends StatefulWidget {
  final DynamicSliderModel controller;
  final FormController formController;

  const DynamicSliderController({
    super.key,
  ...
  @override
  State<DynamicSliderController> createState() =>
      _DynamicSliderControllerState();
}

class _DynamicSliderControllerState extends State<DynamicSliderController> {
  ...
  Widget build(BuildContext context) {
    ...
    DynamicSliderModel? containsAllPrerequisitesDesign;
            ...
            if (remainingPrerequisites.isNotEmpty) {
              ...
              if (style is Map<String, dynamic>) {
                containsAllPrerequisitesDesign =
                    DynamicSliderModel.fromJson(style);
              } else if (style is DynamicSliderModel) {
    ...
    final displayController = DynamicSliderModel(
      ...
      initialPage: containsAllPrerequisitesDesign?.initialPage ??
          widget.controller.initialPage,
      autoPlay: containsAllPrerequisitesDesign?.autoPlay ??
          widget.controller.autoPlay,
      autoPlayInterval: containsAllPrerequisitesDesign?.autoPlayInterval ??
          widget.controller.autoPlayInterval,
      enableInfiniteScroll:
          containsAllPrerequisitesDesign?.enableInfiniteScroll ??
              widget.controller.enableInfiniteScroll,
      interactable: containsAllPrerequisitesDesign?.interactable ??
          widget.controller.interactable,
      enlargeCenterPage: containsAllPrerequisitesDesign?.enlargeCenterPage ??
          widget.controller.enlargeCenterPage,
      viewportFraction: containsAllPrerequisitesDesign?.viewportFraction ??
          widget.controller.viewportFraction,
      aspectRatio: containsAllPrerequisitesDesign?.aspectRatio ??
          widget.controller.aspectRatio,
      reverse:
          containsAllPrerequisitesDesign?.reverse ?? widget.controller.reverse,
      pauseAutoPlayOnTouch:
          containsAllPrerequisitesDesign?.pauseAutoPlayOnTouch ??
              widget.controller.pauseAutoPlayOnTouch,
      pauseAutoPlayInFiniteScroll:
          containsAllPrerequisitesDesign?.pauseAutoPlayInFiniteScroll ??
              widget.controller.pauseAutoPlayInFiniteScroll,
      scrollDirection: containsAllPrerequisitesDesign?.scrollDirection ??
          widget.controller.scrollDirection,
      pageSnapping: containsAllPrerequisitesDesign?.pageSnapping ??
          widget.controller.pageSnapping,
      height:
  ...
  Future<void> handleDynamicOnClick(
      DynamicSliderModel controller, Map<String, dynamic> mergedData) async {
